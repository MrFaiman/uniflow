    ...
    while not shutdown:
        ...
        for sock in readable:
            ...
            if random.random() < DisruptionProbabilities.PACKET_LOSS:
                ...
                if LOG_PACKETS:
                    logger.info("dropped src=%s intended_port=%d", addr, intended_port)
                continue
            ...
            if random.random() < DisruptionProbabilities.BIT_FLIP:
                ...
                if LOG_PACKETS:
                    logger.info(
                        "bit_flip intended_port=%d byte=%d bit=%d",
                        intended_port,
                        byte_index,
                        bit_index,
                    )
            ...
            if random.random() < DisruptionProbabilities.MISROUTING:
                wrong_ports = [port for port in RX_PORT_LIST if port != intended_port]
        ...
        now = time.monotonic()
        if STATS_INTERVAL_SEC > 0 and now - last_stats >= STATS_INTERVAL_SEC:
            log_stats(stats)
            last_stats = now
