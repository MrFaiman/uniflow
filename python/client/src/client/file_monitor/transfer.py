...
def transfer_file(
    ...
    for sender, packet in routed_packets:
        ...
        # Empty files have no FEC symbols: repeat their metadata on the assigned path.
        for _ in range(3 if packet.file_size == 0 else 1):
            send_message(connections[sender], payload)
            sent += 1
