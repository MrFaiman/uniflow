...
class FileMonitor:
    ...
    def get_changed_files(self) -> list[Path]:
        if self.use_watchdog:
            return self._get_changed_files_from_events()
        return self._get_changed_files_from_scan()

    def _get_changed_files_from_events(self) -> list[Path]:
        changed_files: list[Path] = []

        with self._lock:
            candidates = set(self._dirty) | set(self.pending)
            self._dirty.clear()

        for file in sorted(candidates):
            try:
                if not file.is_file():
                    self.note_deleted(file)
                    continue
