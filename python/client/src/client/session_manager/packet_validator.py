...
def packet_is_valid(packet: FilePacket) -> bool:
    ...
    try:
        normalized = normalize_relative_path(packet.file_name)
        if (
            normalized.as_posix() != packet.file_name
            or normalized.parts[0] == ".uniflow"
        ):
            return False
    except ValueError:
        return False
    ...
    if packet.symbol_size == 0 or packet.symbol_size > MAX_SYMBOL_SIZE:
        return False
    if len(packet.data) != packet.symbol_size + 4:
        return False
