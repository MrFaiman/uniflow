...
def test_empty_file_survives_lost_metadata_packets(tmp_path):
    root = tmp_path / "out"
    root.mkdir()
    file = root / "empty.bin"
    file.write_bytes(b"")
    ...
    assert (output / "empty.bin").read_bytes() == b""


def test_delete_is_redundant_across_all_three_paths(tmp_path):
    from client.file_monitor.transfer import transfer_delete
    from client.session_manager.manager import SessionManager

    connections = [CaptureConnection(), CaptureConnection(), CaptureConnection()]
    transfer_delete(tmp_path / "file.txt", connections, tmp_path)
    output = tmp_path / "received"
    output.mkdir()
    (output / "file.txt").write_text("old")
    manager = SessionManager(output)
    # Two paths may lose the DELETE; the remaining path still applies it.
    packets = [decode_messages(connection) for connection in connections]
    assert {batch[0].file_id for batch in packets} == {packets[0][0].file_id}
    manager.handle_packet(packets[2][0])
