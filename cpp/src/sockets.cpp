...
UniqueFd create_unix_server(std::string_view path) {
    ...
    if (::lstat(path_owned.c_str(), &existing) == 0) {
        if (!S_ISSOCK(existing.st_mode)) {
            throw std::runtime_error("refusing to replace non-socket IPC path");
        }
        if (::unlink(path_owned.c_str()) < 0) {
            throw std::runtime_error(std::string("unlink socket failed: ") + std::strerror(errno));
        }
    } else if (errno != ENOENT) {
        throw std::runtime_error(std::string("stat socket failed: ") + std::strerror(errno));
    }
